...
- Outside the `과쯔` style, Mercedes-Benz (`벤츠`) keeps the horizontally scrollable outline-chip model rail in Encar class order. In `과쯔`, use generated white/silver vehicle image cards in the same order and show the no-hyphen Korean labels such as `A클래스`, `C클래스`, and `E클래스` with inventory counts.
- The v2 passenger-car depth is `차량유형 → 제조사 → 모델 → 세대 → 트림 → 차종 헤더`. Every depth begins with `전체`, zero-inventory options are disabled, and a depth with one available option advances automatically. Trim supports multiple selection and a separate `적용 N / 매물 수` action; color remains a top sheet chip rather than a depth rail.
- Never expose chassis code names such as `W177`, `W206`, `G20`, or `F30` in cards, chips, accessibility labels, headers, or listing copy. Render generations as `4세대` plus `19 ~ 현재` or `13 ~ 18년식`, and keep the selected model label consistently hyphen-free (`A클래스`).
- In the `과쯔` style, use a 96px rail with a vertically centered 13px `#656D7A` label and 12px vertical card padding for manufacturer, model, trim, and vehicle-header depths. Manufacturer cards are 68×72 with a 28px mark; model cards are 80×72 with a 48×24 vehicle image, 12px/500 model name, and 11px count. The generation depth is the approved exception: a 112px rail containing 84×88 cards with 12px vertical padding, 48×24 image, generation number, year range, and count. Trim chips are 32px high, and the final header uses a 56×28 vehicle image plus a 15px bold title and 12px year/count line.
- Use only `var(--blue)` and the blue tint `#EEF4FF` for selected depth cards and trim choices; do not copy Guazi green or other benchmark brand colors. Cards use `#F5F7FA`, 8px radius, and 8px gaps. Keep model and generation vehicle images under `public/assets/cars/`, brand marks under `public/assets/brand/`, and preserve every asset folder's `CREDITS.md`.
