...
- On first entry, show the Figma vehicle-category rail in this exact order: `중고차`, `트럭 · 특장`, `바이크`, `캠핑카`, `올드카`, `건설기계`, `부품 · 용품`. Use the exact exported category icons, render `올드카` with the user-supplied AutoScout icon at `public/assets/categories/old-car.svg`, and keep the rail 84px tall with the Figma spacing. Tapping `중고차` replaces that rail with the existing BMW/Mercedes manufacturer selector without leaving the listing screen.
